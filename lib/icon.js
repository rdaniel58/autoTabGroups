// Finds the best available icon for a page and extracts its dominant colour.

import { isNeutral } from "./palette.js";

const ANALYSIS_MAX = 64;      // longest edge we rasterise to
const CENTER_BIAS = 0.45;     // how much edge pixels are discounted
const HUE_BUCKETS = 24;       // 15 degrees each
const CHROMATIC_SAT = 0.18;   // saturation at which a pixel counts as coloured
const CHROMATIC_FLOOR = 0.06; // below this share of coloured pixels, call it neutral
...
function scrapeIdentity() {
  ...
  for (const link of document.querySelectorAll("link[rel]")) {
    ...
    // by specification, so they can only ever report a colour the site does
    // not actually use.
...
async function readManifest(manifestUrl) {
  try {
    ...
    const icons = (Array.isArray(json.icons) ? json.icons : [])
      // A "monochrome" icon is a single-colour glyph on transparency by
      // specification -- it says nothing about the brand colour. These are
 ...
 * just downloading more bytes for the same colours. SVGs go last because
 * createImageBitmap cannot rasterise them inside a service worker.
 */
...
/**
 * Weighted-histogram dominant colour.
 *
 * Icons are mostly one brand colour plus white/black/transparent scaffolding, so
 * pixels are weighted by alpha, saturation and distance from centre, with near-
 * white and near-black heavily discounted. The winning hue bucket -- plus its two
 * neighbours, so a colour straddling a boundary is not split -- is then averaged.
 */
function analysePixels(data, size, lenient) {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      const dx = (x - centre) / (centre || 1);
      const dy = (y - centre) / (centre || 1);
  ...
  // Smooth across neighbouring buckets before picking the winner.
  let bestIndex = 0;
  let bestScore = -1;
...
export async function dominantColorOf(url) {
  ...
  try {
    ...
    // Nearest-neighbour: interpolation would invent colours that appear in the
    // icon nowhere, which is what a dominant-colour histogram must not see.
    ctx.imageSmoothingEnabled = false;
    ctx.drawImage(bitmap, 0, 0, size, size);
    const { data } = ctx.getImageData(0, 0, size, size);

    const rgb = analysePixels(data, size, false) ?? analysePixels(data, size, true);
...
/**
 * Work down the ranked icons until one actually yields a colour.
 *
 * Decoding an icon is not the same as learning a colour from it. Plenty of
